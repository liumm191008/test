...
for sample in "${SAMPLES[@]}"; do
  ${DOCKER_RUN} "${PICARD_IMAGE}" bash -c "set -euo pipefail; mkdir -p '${OUTPUT_DIR}/insert_size/${sample}'; picard CollectInsertSizeMetrics I='${OUTPUT_DIR}/filtered/${sample}/${sample}.filtered.bam' O='${OUTPUT_DIR}/insert_size/${sample}/${sample}.insert_size_metrics.txt' H='${OUTPUT_DIR}/insert_size/${sample}/${sample}.insert_size_histogram.pdf' M=0.5 VALIDATION_STRINGENCY=SILENT"
done
